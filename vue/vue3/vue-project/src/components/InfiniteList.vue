...
const data = reactive<any>({
  ...
  // 列表起始值
  startIdx: 0,
  // 列表滚动过快，数据还没渲染，可能会导致空白的情况
  cacheCount: 5,
});

const listData = computed(() => {
    
    if (data.startIdx > data.cacheCount) {
        data.startIdx = data.startIdx - data.cacheCount;
    }

  let endIdx = data.startIdx + data.displayCount + data.cacheCount;
